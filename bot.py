...
def load_config():
    ...
    if os.environ.get("BOT_TOKEN"):
        ...
        try:
            ...
            cfg["ADMIN_IDS"]        = config.CHAT_IDS  # chi era in CHAT_IDS diventa admin
...
def load_subscribers() -> set:
    ...
    # Prima esecuzione: parte con gli admin già iscritti
...
def get_all_recipients() -> set:
    """Restituisce iscritti + admin (gli admin ricevono sempre)."""
    subs = load_subscribers()
    return subs | set(CONFIG["ADMIN_IDS"])
...
# ---------------------------------------------------------------------------
# Fetch lista atti (senza PDF, per polling veloce)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Fetch PDF per una lista di atti (dentro sessione già aperta)
# ---------------------------------------------------------------------------
async def enrich_with_pdf(client, session_url, items):
    """Recupera tutti gli allegati (MC96/97/98/99) per ogni atto."""
    await client.post(
        session_url,
        content=f"&F=MC01&en={ENTE}".encode(),
        headers=HEADERS
    ...
    for item in items:
        ...
        try:
            ...
            # Cerca tutti i link allegato: MC96, MC97, MC98, MC99
            # I numeri possono essere con o senza virgolette: MC98(1) o MC99('1')
            allegati = []
            for func in ["MC96", "MC97", "MC98", "MC99"]:
                pattern = re.compile(rf"{func}\(")
                tags    = soup2.find_all("a", attrs={"onclick": pattern})
                for tag in tags:
                    # Gestisce sia MC98(1) che MC99('1')
            ...
            if allegati:
                ...
                log.info(f"Atto '{item['title'][:40]}': {len(allegati)} allegato/i")
...
def parse_albo_html(html):
    ...
    for card in cards:
        ...
        items.append({
            ...
            "tipo":     tipo,
            "num_pub":  num_pub,
            "allegati": [],  # lista di {"url": ..., "filename": ...}
        })
...
def format_caption(item):
    ...
    if len(allegati) > 1:
        ...
        lines.append(f"📎 Allegati: 1 documento")
    else:
...
async def send_item_to_chat(bot, chat_id: int, item: dict):
    ...
    allegati = item.get("allegati", [])
    try:
        # Sempre prima il testo, poi i documenti in sequenza
        ...
        log.info(f"✓ Inviato a {chat_id}: {item['title'][:60]} ({len(allegati)} allegati)")
    except Exception as e:
...
async def cmd_abbonati(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if chat_id in subs:
        await update.message.reply_text(
            "✅ Sei già iscritto alle notifiche dell'Albo Pretorio." + MENU_TEXT,
            parse_mode=ParseMode.MARKDOWN
        )
        return
...
async def cmd_disabbonati(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Gli admin non possono disabbonarsi
    if chat_id in CONFIG["ADMIN_IDS"]:
        await update.message.reply_text(
            "ℹ️ Gli amministratori ricevono sempre le notifiche." + MENU_TEXT,
            parse_mode=ParseMode.MARKDOWN
        )
        return
    ...
    if chat_id not in subs:
        await update.message.reply_text(
            "ℹ️ Non eri iscritto alle notifiche." + MENU_TEXT,
            parse_mode=ParseMode.MARKDOWN
        )
        return
...
async def cmd_atti(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        f"✅ Mostrati {len(items)} atti." + MENU_TEXT,
        parse_mode=ParseMode.MARKDOWN
    )
...
async def cmd_controlla(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if new_items:
        ...
        save_seen(new_seen)
        await update.message.reply_text(
            f"✅ {len(new_items)} nuovi atti aggiunti all'archivio." + MENU_TEXT,
            parse_mode=ParseMode.MARKDOWN
        )
    else:
        await update.message.reply_text(
...
async def cmd_unknown(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        "Non riconosco questo tipo di messaggio." + MENU_TEXT,
        parse_mode=ParseMode.MARKDOWN
    )
